...
def cce_forward_multimodal(
    ...
    **kwargs,
) -> Union[tuple, Qwen4ExpCausalLMOutputWithPast]:
...
def patch_qwen4_exp_text(
    ...
    if remote_model_id is not None:
        patch_remote_model_class(
            ...
            class_name="Qwen4ExpForCausalLM",
    ...
    if isinstance(maybe_model, transformers.PreTrainedModel):
        assert isinstance(maybe_model, modeling_qwen4_exp.Qwen4ExpForCausalLM), (
            f"Expected a Qwen4ExpForCausalLM model. Got {type(maybe_model)}."
        )
        maybe_model.forward = MethodType(cce_forward, maybe_model)
        return maybe_model

    modeling_qwen4_exp.Qwen4ExpForCausalLM.forward = cce_forward
    return None
...
def patch_qwen4_exp(
    ...
    if remote_model_id is not None:
        patch_remote_model_class(
            remote_model_id=remote_model_id,
            class_name="Qwen4ExpForConditionalGeneration",
    ...
    if isinstance(maybe_model, transformers.PreTrainedModel):
        assert isinstance(maybe_model, modeling_qwen4_exp.Qwen4ExpForConditionalGeneration), (
            f"Expected a Qwen4ExpForConditionalGeneration model. Got {type(maybe_model)}."
        )
        maybe_model.forward = MethodType(cce_forward_multimodal, maybe_model)
        return maybe_model

    modeling_qwen4_exp.Qwen4ExpForConditionalGeneration.forward = cce_forward_multimodal
    return None
