...
def patch_glm_moe_dsa(
    ...
    if isinstance(maybe_model, transformers.PreTrainedModel):
        assert isinstance(maybe_model, modeling_glm_moe_dsa.GlmMoeDsaForCausalLM), (
            f"Expected a GlmMoeDsaForCausalLM model. Got {type(maybe_model)}."
        )
        maybe_model.forward = MethodType(cce_forward, maybe_model)
        return maybe_model

    modeling_glm_moe_dsa.GlmMoeDsaForCausalLM.forward = cce_forward
    return None
